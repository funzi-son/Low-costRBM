function model = train_rbm_l(conf,dat,lab)
...
%        acc_e = acc_e + sum(sum(labP == labN))/sNum;
        ...
        %% Sparsity contrains
        if conf.lambda >0
           hidI = (visP*model.W + model.U(labP,:) + repmat(model.hidB,sNum,1));
           hidN = logistic(hidI);
           pppp = (conf.p - sum(hidN,1)/sNum);
           model.W    = model.W   + lr*conf.lambda*(repmat(pppp,visNum,1).*(visP'*((hidN.^2).*exp(-hidI))/sNum));
           model.U    = model.U   + lr*conf.lambda*(repmat(pppp,labNum,1).*(s_labP'*((hidN.^2).*exp(-hidI))/sNum));
