function model = train_rbm_l_(conf,dat,lab,testd,testl,log_file)
...
%        acc_e = acc_e + sum(sum(labP == labN))/sNum;
    ...
    for prune = [800] %[300 400 500 600 700 800 900 1000]
        pNums = [prune];
    for gen_c = conf.gen_c
    if gen_c==0
        cpen = 1;
    else 
        cpen = 0;
    ...
    [T R] = extract_rbm_l(model,'',0,cpen);
   
    Rs(1) = R;
        
    [T,Rs] = prune_rule(T,Rs,pNums);
    
    if conf.gen_c>0, [T,Rs] = generalize_rule(T,Rs,gen_c); end
    routput = rule_inference(Rs,T,testd);
    racc = sum(sum(routput==testl+1))/size(testl,1);   
    logging(log_file,[e gen_c pNums macc_e macc_r racc]);
